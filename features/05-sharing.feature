Feature: Sharing
  ...
  Scenario: Bob can grant read access to John only
    ...
    """
    [ { "_id" : "541816f042e7d8204d000002", "label" : "Army Camp", "level" : 3, "_owner" : "john@doe.com" }, 
      { "_id" : "541816f042e7d8204d000001", "label" : "Town Hall", "level" : 3, "_owner" : "bob@sponge.com", "_tags": [{ "_targets": ["john@doe.com"], "_permissions": [{"_read": true}]}] } ]
    """
    When client is authenticated as Tom
    And client requests GET /api/buildings/541816f042e7d8204d000001
    Then response status should be 422
  ...
  Scenario: Bob can grant read/write access to John only
    Given the system only knows those Buildings:
    | _id                      | label     | level | _owner         |
    | 541816f042e7d8204d000001 | Town Hall |     3 | bob@sponge.com |
    | 541816f042e7d8204d000002 | Army Camp |     3 | john@doe.com   |
  ...
  Scenario: John must not be able to see accesses granted by Bob to other users
    Given the system only knows those Buildings:
    ...
    { "_id" : "541816f042e7d8204d000001",
      ...
      "level" : 3,
      "_owner" : "bob@sponge.com",
      "_tags": [{ "_targets": ["john@doe.com"], "_permissions": [{"_read": true},{"_write":true}]}, { "_targets": ["public"], "_permissions": [{"_read": true}]}] }
    """
